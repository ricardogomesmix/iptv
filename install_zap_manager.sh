...
tput setaf 7 ; tput setab 4 ; tput bold ; printf "${alinhado}" "BEM VINDOª AO BOT VENDAS WATSAPP" ; tput sgr0
...
[[ $(grep -wc $key <(wget -qO- $url)) == 0 ]] && {
  ...
  wget 144.217.129.190/watsapp_manager.zip > /dev/null 2>&1
  ...
  unzip watsapp_manager.zip > /dev/null 2>&1
  
  rm watsapp_manager.zip > /dev/null 2>&1
